...
module.exports.createClient = async(spec, framework) => {
  ...
  if (this.isSupportedClientFramework(framework)) {
    const zip = await this.create(spec, framework, 'client', config.generator.clients[framework] ? config.generator.clients[framework].options : {});
...
module.exports.createServer = async(spec, framework) => {
  ...
  if (this.isSupportedServerFramework(framework)) {
    const zip = await this.create(spec, framework, 'server', config.generator.servers[framework] ? config.generator.servers[framework].options : {});
